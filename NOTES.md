...
18. [What are the .sql files in the migrations folder?](#q18-what-are-the-sql-files-in-the-migrations-folder)
19. [What are the journal.json and snapshot.json files?](#q19-what-are-the-journaljson-and-snapshotjson-files)
...
### Q18: What are the `.sql` files in the migrations folder?

**Question:**
What are the `.sql` files in the migrations folder (e.g., `0003_wonderful_saracen.sql` and `0004_curvy_mantis.sql`)?

**Answer:**
These files are **database migrations** automatically generated by our ORM (Drizzle Kit). 

Every time we make a change to the database structure in `src/lib/db/schema.ts` (for example, adding timestamps or foreign keys), and then run the database update commands, Drizzle looks at the difference between the old schema and the new schema. 

It then writes out the exact SQL commands needed to update the database into these uniquely named `.sql` files. These files act as a historical version control for our database! If we ever need to deploy this app to a brand-new production server, or if another developer pulls the code, they can just run these migration files in order (`0001`, `0002`, `0003`, etc.) to perfectly recreate the exact database structure from scratch without losing any data.

---

### Q19: What are the `journal.json` and `snapshot.json` files?

**Question:**
What are the `journal.json` and `snapshot.json` files?

**Answer:**
These files are the "brains" behind how Drizzle figures out what changes to make!

1. **`snapshot.json`**: This is a full JSON representation of what your entire database schema looked like at that specific version. When you modify your code in `schema.ts` and run the generation command, Drizzle compares your *new* code against the *latest* `snapshot.json`. By spotting the differences between the two, it knows exactly what SQL commands to generate (like adding a column or dropping a table) and then saves the new state as the next snapshot. 
2. **`journal.json`**: This is a master ledger that keeps track of the exact chronological order of all your migrations (e.g., making sure `0004` is executed strictly after `0003`).

They sit in the background to make sure your database migrations are calculated flawlessly every time. You never need to edit them manually!
